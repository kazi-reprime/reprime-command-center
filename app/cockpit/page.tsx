...
import type { SeedClient, SeedLead, SeedTask, SeedAgent, SeedAutomation, SeedMessage } from '@/lib/data/seed'
...
export default function CockpitDashboard() {
  ...
  // Use API data when available, otherwise fall back to seed data
  ...
  const m = computeMetrics() // still using seed metrics function for KPIs
  const priorities = tasks
    .filter(t => t.status !== 'done')
    .sort((a, b) => a.priority - b.priority)
    .slice(0, 4)

  const recentMessages = messages.filter(msg => !msg.isRead).slice(0, 3)
  const hotLeads = leads.filter(l => l.score >= 80 && !['won', 'lost'].includes(l.stage)).slice(0, 3)
  const errorAgents = agents.filter(a => a.status === 'error' || a.errorCount > 0)
  const failedAutos = automations.filter(a => a.status === 'error')
      ...
      {isLoading ? <LoadingState message="Loading dashboard data..." /> : (
        <>
            ...
            <div style={{ flex: 1, display: 'flex', gap: '1.5rem', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              ...
              {recentMessages.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: '#3B82F6', fontSize: '0.75rem', fontWeight: 600 }}>
                  <span>💬</span> {recentMessages.length} unread message{recentMessages.length > 1 ? 's' : ''}
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{
            ...
            <StatCard label="Active Clients" value={clients.filter(c => c.status === 'active').length} change={2} changeLabel="+2 this month" icon={<span>👥</span>} color="#00A980" />
            <StatCard label="New Leads" value={leads.filter(l => l.stage === 'new').length} change={-5} icon={<span>🎯</span>} color="#A855F7" />
            <StatCard label="Conversion Rate" value={`${m.conversionRate}%`} change={3} icon={<span>🔄</span>} />
            <StatCard label="AI Agents Active" value={`${agents.filter(a => a.status === 'running').length}/${agents.length}`} changeLabel="running" icon={<span>🤖</span>} color="#3B82F6" />
          </div>
          ...
          <div style={{
            ...
            <p style={{ margin: 0, color: '#e2e8f0', fontSize: '0.85rem', lineHeight: 1.6 }}>
              <strong style={{ color: '#FFCC33' }}>Priority:</strong>{' '}
              {hotLeads.length > 0
                ? `${hotLeads[0].name} (${hotLeads[0].business}) has a high lead score of ${hotLeads[0].score}. Next action: ${hotLeads[0].nextAction}. `
                : 'No hot leads requiring immediate attention. '
              }
              {priorities.length > 0
                ? `Focus on "${priorities[0].title}" — it's your top priority task due ${priorities[0].dueDate || 'soon'}.`
