...
export default function CockpitDashboard() {
  ...
  const systemServices = (() => {
    if (!healthData?.integrations) return [
      { label: 'Nora AI', status: 'Checking...', color: 'text-slate-400', bg: 'bg-slate-50' },
      { label: 'WhatsApp', status: 'Checking...', color: 'text-slate-400', bg: 'bg-slate-50' },
      { label: 'Gmail', status: 'Checking...', color: 'text-slate-400', bg: 'bg-slate-50' },
      { label: 'Database', status: 'Checking...', color: 'text-slate-400', bg: 'bg-slate-50' },
    ]
    ...
    const getStatus = (name: string) => {
      ...
      if (!found) return { status: 'N/A', color: 'text-slate-400' }
      return found.status === 'connected'
        ? { status: 'Live', color: 'text-emerald-500' }
        : found.status === 'error'
        ? { status: 'Error', color: 'text-red-500' }
        : { status: 'Not Set', color: 'text-amber-500' }
    }
    return [
      { label: 'Nora AI', ...getStatus('Anthropic'), bg: 'bg-slate-50' },
      { label: 'WhatsApp', ...getStatus('WhatsApp'), bg: 'bg-slate-50' },
      { label: 'Gmail', ...getStatus('Gmail'), bg: 'bg-slate-50' },
      { label: 'Database', ...getStatus('Database'), bg: 'bg-slate-50' },
    ]
  })()
            ...
            <div className="glass-card rounded-3xl p-8 relative overflow-hidden group border-l-4 border-l-blue-500">
              ...
              <div className="flex justify-between items-start relative z-10 mb-8">
                ...
                <button 
                  ...
                  className="h-12 px-6 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold tracking-wide transition-all shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 flex items-center gap-2 cursor-pointer"
                >
                  Execute
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
                <div className="p-4 rounded-2xl bg-white/50 border border-slate-100">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Top Deal</div>
                  <div className="text-sm font-semibold text-slate-800">{topDeal?.name || 'None'}</div>
                </div>
                <div className="p-4 rounded-2xl bg-white/50 border border-slate-100">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Status</div>
                  <div className="text-sm font-semibold text-slate-800">{topDeal?.status || 'N/A'}</div>
                </div>
                <div className="p-4 rounded-2xl bg-white/50 border border-slate-100">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Value</div>
                  <div className="text-sm font-semibold text-slate-800">{topDeal?.purchasePrice ? fmt(topDeal.purchasePrice) : 'TBD'}</div>
                </div>
                <div className="p-4 rounded-2xl bg-white/50 border border-slate-100">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Tasks</div>
                  <div className="text-sm font-semibold text-slate-800">{priorities.length} Pending</div>
                </div>
              </div>
            </div>
            ...
            <Card title="Communications">
              <div className="grid grid-cols-2 gap-3 mb-4">
                <Link href="/cockpit/comms" className="p-4 rounded-2xl bg-green-50 hover:bg-green-100 border border-green-100 transition-colors flex flex-col items-center justify-center text-center gap-2">
                  ...
                  <div className="font-bold text-green-900 text-sm">WhatsApp</div>
                  <div className="text-[10px] font-bold text-green-600/70 uppercase tracking-widest">View Threads</div>
                </Link>
                <Link href="/cockpit/email" className="p-4 rounded-2xl bg-red-50 hover:bg-red-100 border border-red-100 transition-colors flex flex-col items-center justify-center text-center gap-2">
                  ...
                  <div className="font-bold text-red-900 text-sm">Gmail</div>
                  <div className="text-[10px] font-bold text-red-600/70 uppercase tracking-widest">View Inbox</div>
                </Link>
              </div>
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-widest">Pending Tasks</div>
                  <span className="w-5 h-5 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-[10px] font-bold">{priorities.length}</span>
                </div>
